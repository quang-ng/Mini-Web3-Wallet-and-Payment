...
class WalletService {
  async getBalance(userAddress: string): Promise<string> {
    try {
      ...
      const balance = await paymentVaultcontract.getBalance(userAddress);

      logger.debug("WalletService", "Raw balance:", {
        balance,
        type: typeof balance,
      });
  ...
  async getSimpleTokenBalance(userAddress: string): Promise<string> {
    try {
      const simpleTokenBalance = await paymentVaultcontract.getTokenBalance(
        userAddress,
        config.simpleTokenAddress,
      );

      const balanceStr = ethers.formatUnits(simpleTokenBalance, 18);
